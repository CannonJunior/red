...
| Proposal Writing Module | 2026-04-09 | proposal/drafts_db.py (4-table schema + full CRUD), drafts_templates.py (9 doc types), llm_writer.py (5 actions), server/routes/proposal_drafts.py (26 endpoints), routes registered, js/proposal-writer.js, js/proposal-comments.js, index.html "Proposal Docs" nav + panel; 32 tests passing |
| Color Team Review + Compliance Assessment | 2026-04-09 | proposal/color_team.py (docx comment extraction, section matching, LLM requirement detection, FAR 15.305 rating), server/routes/color_team.py (upload + assessment endpoints), proposal-writer.js UI (upload modal, compliance panel, rating badges); 24 tests passing |
| Proposal writing + color team improvements | 2026-04-09 | 8 items: (1) assert→RuntimeError in drafts_db.py, (2) hardcoded llama3→model_config.py LOCAL tier in llm_writer+color_team, (3) cgi.FieldStorage→email.message multipart parser, (4) opportunity_id wired navigation.js→ProposalWriter.init→color team upload, (5) stage-snapshot endpoint + UI modal, (6) proposal/drafts_exporter.py + GET /export route + Export DOCX button, (7) export_color_team_docx() + GET /color-team-export route + CT Export button, (8) compliance_check auto-fetches shredding requirements from /api/shredding/requirements/{opportunity_id} |
